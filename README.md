...
2. Pushes image to ECR tagged with Git commit SHA
3. Updates k8s/deployment.yaml with new image tag
4. Commits the update back to the repository

### CD Pipeline
Triggers when CI pipeline completes or on changes to k8s/. Steps:
1. Configures kubectl to connect to EKS
2. Applies updated Kubernetes manifests
3. Waits for rollout to complete successfully
